...
export const GeodataSection: React.FC<GeodataSectionProps> = ({
  geodata,
  onEdit,
  onDelete,
  enabled,
    ...
    <div className={`bg-slate-900/60 border border-white/10 rounded-2xl p-5 backdrop-blur-xl shadow-xl space-y-3 transition-opacity ${isEnabled ? '' : 'opacity-60'}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-semibold text-base text-white">地理数据</h3>
            <p className="text-xs text-slate-400">
              自动更新: <span className="font-mono text-emerald-300">{geodata?.autoUpdate !== false ? '开启' : '关闭'}</span> | 周期: <span className="font-mono text-slate-200">{geodata?.interval || '24h'}</span>
            </p>
          </div>
        </div>

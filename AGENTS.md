...
| `outbounds` | `Array` | 出站协议对象列表（`vless`, `vmess`, `trojan`, `shadowsocks`, `hysteria`, `wireguard`, `socks`, `http`, `dns`, `freedom`, `blackhole`） |
...
> **重要**：新版 Xray 出站 `settings` 采用**扁平的 OutboundConfigurationObject 结构**（simplified outbound），不再使用旧版的 `vnext` / `servers` 数组嵌套。生成配置时必须使用以下扁平结构；仅在**读取**用户旧配置时兼容 `vnext` / `servers` 格式。

- **VLESS (`protocol: "vless"`)**
  - 设置: `{ "address": string, "port": number, "id": string, "encryption": "none", "flow": "" | "xtls-rprx-vision" | "xtls-rprx-vision-udp443", "level"?: number, "reverse"?: { "tag": string, "sniffing"?: {} } }`
  - *注意：用户标识字段为 `id`，非 `uuid`；`encryption` 不能留空，禁用需显式设为 `"none"`*
- **VMess (`protocol: "vmess"`)**
  - 设置: `{ "address": string, "port": number, "id": string, "security": "auto" | "aes-128-gcm" | "chacha20-poly1305", "experiments"?: string, "level"?: number }`
  - *注意：`alterId` 已被 Xray 废弃移除，禁止生成*
- **Trojan (`protocol: "trojan"`)**
  - 设置: `{ "address": string, "port": number, "password": string, "email"?: string, "level"?: number }`
- **Shadowsocks (`protocol: "shadowsocks"`)**
  - 设置: `{ "address": string, "port": number, "method": string, "password": string, "email"?: string, "level"?: number }`
- **Hysteria2 (`protocol: "hysteria"`)**
  - 设置: `{ "version": 2, "address": string, "port": number }`
  - *注意：协议名为 `hysteria`（不存在 `hysteria2` 协议名），`version` 必须为 2；认证密码位于 `streamSettings.hysteriaSettings.auth`，底层传输需配置 `network: "hysteria"` + TLS*
- **SOCKS (`protocol: "socks"`)**
  - 设置: `{ "address": string, "port": number, "user"?: string, "pass"?: string }`
- **HTTP (`protocol: "http"`)**
  - 设置: `{ "address": string, "port": number, "user"?: string, "pass"?: string, "headers"?: {} }`
...
- **`network`** (底层传输方式，新版文档中同义字段为 `method`): `"tcp"` \| `"ws"` \| `"grpc"` \| `"h2"` \| `"quic"` \| `"hysteria"`
...
- **`hysteriaSettings`**: `{ "version": 2, "auth": string, "udpIdleTimeout"?: number, "masquerade"?: {} }`（搭配 `hysteria` 协议使用）
...
│   ├── components/               # 通用组件 (CustomSelect, StreamSettingsPanel, TunSettingsModal 等)
│   ├── pages/                    # 页面 (JsonConfig, Logs, Settings)
│   │   ├── config-modals/        # 各配置模块的编辑弹窗 (OutboundModal, InboundModal, SubscriptionImportModal 等)
│   │   └── config-sections/      # 各配置模块的展示区块 (OutboundSection, RoutingSection 等)
│   ├── stores/                   # 状态管理
...
│   │   └── useUpdateStore.ts     # 自动更新状态
│   ├── types/                    # TypeScript 类型定义
│   │   └── index.ts
│   └── utils/                    # 工具函数 (configSectionHelper, date, logParser)
└── src-tauri/                    # 后端（Rust / Tauri 2）
    └── src/
        ├── config/               # Rust 配置处理模块
        │   ├── builder.rs        # 运行时 config.json 程序化构建器
        │   ├── parser.rs         # 订阅/分享链接解析（生成符合官方规范的出站配置）
...
1. **配置文件 Store (`src/stores/useConfigStore.ts`)**：
   - 维护多个配置预设（每个预设包含一份完整的 `config.json` 文本）。
   - 预设模板严格遵循 `https://xtls.github.io/config`。
2. **出站编辑与订阅导入**：
   - 出站结构的生成与编辑集中在 `src/pages/config-modals/OutboundModal.tsx`，生成的 `settings` 必须符合上文「出站协议映射」的扁平 OutboundConfigurationObject 结构。
   - 订阅/分享链接解析位于 Rust 后端 `src-tauri/src/config/parser.rs`，同样按扁平结构生成出站配置；读取用户旧配置时须兼容 `vnext` / `servers` 旧格式。
...
### 4. 代码类型与安全规范
- 修改出站/入站结构生成逻辑（如 `OutboundModal.tsx`、`parser.rs`、`builder.rs`）时，必须同步检查并更新 `src/types/index.ts` 中的 TypeScript 接口定义。
...
- **统一使用自定义下拉组件**：必须统一使用 `CustomSelect`（`src/components/CustomSelect.tsx`）。
- **统一设计风格**：下拉菜单需采用深色半透明玻璃材质（`bg-slate-900/98 backdrop-blur-2xl`）、细微边框高亮、选中项 Check 标识、圆角与平滑过渡动画，保持与分流规则中的出站目标下拉框完全统一的视觉与交互规范。
